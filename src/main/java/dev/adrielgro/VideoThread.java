    ...
    public void run() {
        ...
        for (int i = 0; i < DAHUA_ACCOUNTS.length; i++) { // Vamos a recorrer todas las posibles cuentas, hasta poder acceder
            ...
            while(errPacket == null) { // mientras no existan errores, avanzamos a la siguiente camara de la misma IP
                ...
                int rndWidth = rand.nextInt((int)screenSize.getWidth()-320); // Creamos un random entre 0 y el ancho de la pantalla
                int rndHeight = rand.nextInt((int)screenSize.getHeight()-320); // Creamos un random entre 0 y la altura de la pantalla
                ...
                try {
                    ...
                    System.out.println("Error al conectarse a: " + mediaReader.getUrl() + ", msg: " + e.getMessage());
